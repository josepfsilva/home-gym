...
function loadPlanHead_2(planNumber) {

    return fetch('/planotreino/' + planNumber)
        .then(response => {
            if (!response.ok) {
...
function loadPlanInfo_2(planNumber) {
    return fetch('/planotreino/' + planNumber)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
        ...
        .then(data => {
            ...
            function sendToVoice(texto) {
                ...
                var result = speak;
                mmiCli_1.sendToIM(new LifeCycleEvent("APPSPEECH", "IM", "text-1", "ctx-1").doStartRequest(new EMMA("text-", "text", "command", 1, 0).setValue(JSON.stringify(result))));
            }
            ...
            html += '<h3>' + planDetails[0] + '</h3>';
            html += '<div class="horizontal-line"></div>';
            html += '<h3 class="PlanNumber">Plano ' + planNumber + '</h3> <div class="streak_level_group"><div id="level"></div><div id="streak"></div></div></div>'; //lev
            html += '<div id="mic"> <svg class="mic" id="mic" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style="width:70px; height:70px;"> <path fill="#000000" d="M12,1A4,4 0 0,1 16,5V11A4,4 0 0,1 12,15A4,4 0 0,1 8,11V5A4,4 0 0,1 12,1M12,19A7,7 0 0,0 19,12H22A10,10 0 0,1 12,22A10,10 0 0,1 2,12H5A7,7 0 0,0 12,19Z" /></svg> </div>'; //m
            html += '<p id="datetime" class="datetime-container"><span class="time"></span><span class="date"></span></p></div></div>';
            // fim do cabeçalho
            //inicio dos detalhes
...
function getPlanName_2(planNumber) {
    return fetch('/planotreino/' + planNumber)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
...
function loadExercise_2(planNumber, count) {
    return fetch('/planotreino/' + planNumber)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
        ...
        .then(data => {
            ...
            container.empty();
            var exercises = data[1]
            var totalExercises = exercises.length; // Get the total number of exercise
...
function getPlanDuration_2(planNumber) {
    return fetch('/planotreino/' + planNumber)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
